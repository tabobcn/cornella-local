-- ====================================================
-- TABLA: offers (Ofertas y promociones de negocios) - FIXED
-- ====================================================

-- IMPORTANTE: Este archivo corrige el tipo de business_id de uuid a integer
-- para que coincida con el tipo de businesses.id (serial/integer)

CREATE TABLE IF NOT EXISTS public.offers (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  business_id integer NOT NULL REFERENCES public.businesses(id) ON DELETE CASCADE, -- FIXED: uuid → integer
...
-- Política NUEVA: Propietarios pueden VER TODAS sus ofertas (incluso pausadas/invisibles)
...
CREATE POLICY "Propietarios pueden crear ofertas"
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE businesses.id = offers.business_id
      AND businesses.owner_id = auth.uid()
      AND businesses.is_verified = true -- Requiere negocio verificado
    )
  );
...
COMMENT ON COLUMN public.offers.business_id IS 'ID del negocio (integer) que crea la oferta';
COMMENT ON COLUMN public.offers.discount_type IS 'Tipo de descuento: percentage, fixed_amount, 2x1, free, custom';
COMMENT ON COLUMN public.offers.is_flash IS 'true = oferta flash (8h), false = oferta normal (3+ días)';
COMMENT ON COLUMN public.offers.status IS 'Estado: active, paused, expired';
